...
const MiniButton = ({ label, onClick, className = "" }: KeyPadButtonProps) => {
  const baseClassName = "text-white text-sm p-1 bg-dark2 rounded-md w-16";
...
const BigButton = ({ label, onClick, className = "" }: KeyPadButtonProps) => {
  const baseClassName =
    "text-center text-white text-2xl hover:bg-dark2 rounded-xl h-16 mb-2 transition-colors";
  const processedClassName = cn(baseClassName, className);
  return (
    <button className={processedClassName} onClick={onClick}>
      {label}
...
const BackButton = ({ onClick, className = "" }: KeyPadButtonProps) => {
  const baseClassName =
    "flex flex-row justify-center items-center hover:bg-dark2 rounded-xl h-16 mb-2 transition-colors";
  const processedClassName = cn(baseClassName, className);
  return (
    <button className={processedClassName} onClick={onClick}>
...
export const Keypad = ({
  ...
  switch (type) {
          ...
          <div className="flex flex-row flex-wrap !gap-0">
            {numberList.map((value) =>
              value >= 0 ? (
                ...
                <div
                  key={value}
                  className="flex flex-col items-center justify-center w-3/12 text-primary opacity-50"
                >
    ...
    case 3:
      return (
        <div className="flex felx-col mt-8">
